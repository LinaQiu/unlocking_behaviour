...
##Define output path
OUTPUT_PATH ="/Users/lina/Desktop/Ahmed/OUTPUT/"
	...
	if not csvFilename.endswith('.csv'):
		...
		try:
			with open(INPUT_PATH+csvFilename,'rb') as read_file_obj:
				...
				for row in reader:
					if (row[0]=='Screen On'):
						row.append('user'+str(count))
						next_row=next(reader)
						next_row.append('user'+str(count))
						...
						##Otherwise, see if the next row begins with 'Unsuccessful', which means it might be a one/two_failure_auth attempt 
							...
							next_2_row.append('user'+str(count))
							...
							##See if it's two_failure_auth attempt
								...
								next_3_row.append('user'+str(count))
